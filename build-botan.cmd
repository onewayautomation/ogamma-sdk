...
python configure.py --debug-mode --enable-static-library --prefix=%REPO_BASE_FOLDER%\%PATH_BOTAN%\install\debug --cxxflags=/std:c++latest
nmake BUILD=debug
nmake install
:BUILD_BOTAN_RELEASE
IF EXIST %REPO_BASE_FOLDER%\%PATH_BOTAN%\install\release\botan.lib GOTO END_BUILD_BOTAN
python configure.py --enable-static-library --prefix=%REPO_BASE_FOLDER%\%PATH_BOTAN%\install\release --cxxflags=/std:c++latest
nmake BUILD=release
nmake install
